...
//Adds matrix M to mfile setting it to variable varname
void d_matlab_print(matrix_t *M, char *mfile, char *varname) {
	FILE *f;
	int i, j;
	precision val;
	
	f = fopen(mfile, "a");
	fprintf(f, "%s = round([", varname);
...
// Adds function to mfile for matlab to execute
void d_matlab_function(char *function, char *fname, int iteration, char *mfile)
{
	FILE *f;
	f = fopen(mfile, "a");
	fprintf(f, "%% %s test %d \n", fname, iteration);
	fprintf(f, "%s\r\n", function);
	fprintf(f, "if (~isequal(c,d))\n");
	fprintf(f, "disp 'Failed %s test %d.'\n", fname, iteration);
	fprintf(f, "dbstack\n");
	fprintf(f, "end\n");
	fclose(f);
}

// Randomly fills matrix M with integers ranging from 0-100000
void d_generateRandomMatrix(matrix_t *M) {
	int i, j;
...
matrix_t * m_initialize (int mode, int numRows, int numCols) {
	...
	if(M == NULL) {
		printf("m_initialize could not allocate %lu bytes for matrix_t structure.\n", sizeof(matrix_t));
		exit(-1);
	}
	M->numRows = numRows;
	M->numCols = numCols;
	...
	if (mode == ZEROS || mode == IDENTITY) {
		...
		if(M->data == NULL) {
			printf("m_initialize could not allocate %lu bytes for M->data.\n", numRows * numCols * sizeof (precision));
			exit(-1);
		}
		if (mode == IDENTITY) {
...
matrix_t * m_copy (matrix_t *M) {
	...
	if(C == NULL) {
		printf("m_copy could not allocate %lu bytes for matrix_t structure.\n", sizeof(matrix_t));
		exit(-1);
	}
	C->numRows = M->numRows;
	C->numCols = M->numCols;
	...
	if(C->data == NULL) {
		printf("m_copy could not allocate %lu bytes for C->data.\n", C->numRows * C->numCols * sizeof (precision));
		exit(-1);
	}
	if (M->numCols == M->span) {
